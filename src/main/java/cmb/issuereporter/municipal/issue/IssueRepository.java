...
public interface IssueRepository extends JpaRepository<Issue, Integer> {
    ...
    @Query("SELECT i FROM Issue i WHERE (:areaId is null or i.area.id = :areaId) and (:categoryId is null or i.category.id = :categoryId) and " +
            ...
            "(:assigneeId is null or i.assignee.id = :assigneeId) and (:assignById is null or i.assignBy.id = :assignById) and Date(i.createdDate) >= :startDate AND Date(i.createdDate) <= :endDate")
